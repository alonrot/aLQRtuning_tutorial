...
% tuning dw.Q, we should try to make J(dw) close to J_best
    [dw.Q,dw.R] = get_LQR_weights([1 1 1 1]);
    
%% Simulate pole dynamics:

% Design LQR control gain: note that we use the linearized model [A,B],
% that is computed using wrong pole parameters, and also the design weights
% dw.Q and dw.R, instead of the empirical weights ew.Q, ew.R
...
% Comment out this cell if undesired
	...
	[fig_hdlb,hdl_splotb] = initialize_plot();
    plot_cart_pole(fig_hdlb,hdl_splotb,x_simub,F_simub,sims);
