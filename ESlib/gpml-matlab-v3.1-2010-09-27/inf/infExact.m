function [post nlZ dnlZ] = infExact(hyp, mean, cov, lik, x, y,delta)
...
[n, D] = size(x);
if ~isempty(delta)
    K = feval(cov{:}, delta, hyp.cov, x);               % evaluate covariance matrix
    m = feval(mean{:}, delta, hyp.mean, x);                   % evaluate mean vector
else
...
if nargout>1                               % do we want the marginal likelihood?
  ...
  if nargout>2                                         % do we want derivatives?
    ...
    for i = 1:numel(hyp.cov)
      if ~isempty(delta)
          dnlZ.cov(i) = sum(sum(Q.*feval(cov{:}, delta, hyp.cov, x, [], i)))/2;
      else
